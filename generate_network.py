...
def main():
    ...
    for paper in all_results:
        network.graph.add_node(paper["paper_name"], 
                              title=paper.get("insights", "").split("\n")[1] if paper.get("insights") else "Unknown",
    ...
    for i, paper1 in enumerate(all_results):
        for paper2 in all_results[i+1:]:
            similarity = network.compute_similarity(paper1.get("insights", ""), paper2.get("insights", ""))
            if similarity > 0.3:  # Lower threshold to get more relationships
                relationships.append({
                    "paper1": paper1["paper_name"],
                    "paper2": paper2["paper_name"],
                    "similarity": similarity,
                    "shared_topics": network._find_shared_topics(paper1.get("insights", ""), paper2.get("insights", ""))
                })
    
    # Create network analysis results
    network_results = {
        "num_papers": len(all_results),
        "num_citations": len([n for n in network.graph.nodes() if n.startswith("Citation:")]),
    ...
    print(f"- Found {network_results['num_relationships']} relationships between papers")
