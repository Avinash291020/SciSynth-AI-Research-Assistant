...
# Page config
st.set_page_config(
    page_title="SciSynth: AI Research Assistant", page_icon="🧪", layout="wide"
)
...
if results_path.exists():
    try:
        ...
        if (
            ...
            or st.session_state.orchestrator is None
            or getattr(st.session_state.orchestrator, "papers", None) != all_results
        ):
...
if st.session_state.orchestrator:
    ...
    st.sidebar.metric(
        "Active Systems",
        len([s for s in status["systems_status"].values() if s == "ready"]),
    )
else:
...
if page == "🏠 Overview & AI Stack":
    st.markdown(
        '<div class="main-header"><h1>SciSynth: AI Research Assistant (Professional Edition)</h1></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        """
    ...
    **What makes this advanced?**
    ...
    if Path("results/citation_network.png").exists():
        st.image(
            "results/citation_network.png",
            caption="Citation Network Example",
            use_container_width=True,
        )

    st.markdown(
        '<div class="info-box">Explore each section from the sidebar to see advanced AI in action.</div>',
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    if "orchestrator" in st.session_state and st.session_state.orchestrator:
        ...
        with col2:
            papers_with_insights = sum(
                1 for p in st.session_state.all_results if p.get("insights")
            )
            st.metric("Papers with Insights", papers_with_insights)
        with col3:
            papers_with_hypotheses = sum(
                1 for p in st.session_state.all_results if p.get("hypotheses")
            )
            st.metric("Papers with Hypotheses", papers_with_hypotheses)
        with col4:
            active_systems = len(
                [s for s in capabilities.values() if s["status"] == "active"]
            )
            st.metric("Active AI Systems", active_systems)
    else:
        st.markdown(
            '<div class="warning-box">Orchestrator not initialized. Process research collection to enable full dashboard.</div>',
            unsafe_allow_html=True,
        )
        st.warning(
            "Orchestrator not initialized. Process research collection to enable full dashboard."
        )
...
# --- INDIVIDUAL PAPER ANALYSIS ---
    ...
    if (
        ...
        and not st.session_state.processor_initialized
    ):
        try:
                    ...
                    if result:
                        ...
                        with tab1:
                            ...
                            if title:
                                ...
                                st.warning(
                                    "⚠️ Title could not be extracted from the PDF"
                                )
                            ...
                            if metadata.get("technical_terms"):
                                ...
                                if len(terms) > 0:
                                    # Group terms by type
                                    abbreviations = [
                                        t for t in terms if len(t) <= 10 and t.isupper()
                                    ]
                                    algorithms = [
                                        t
                                        for t in terms
                                        if "algorithm" in t.lower()
                                        or "algo" in t.lower()
                                    ]
                                    models = [t for t in terms if "model" in t.lower()]
                                    frameworks = [
                                        t for t in terms if "framework" in t.lower()
                                    ]
                                    other_terms = [
                                        t
                                        for t in terms
                                    ...
                                    # Show total count
                                    st.info(
                                        f"📊 Total technical terms extracted: {len(terms)}"
                                    )
                                else:
                        ...
                        with tab3:
                            ...
                            for hypothesis in hypotheses:
                                if hypothesis.strip() and not hypothesis.startswith(
                                    "Research Hypotheses:"
                                ):
...
# --- RESEARCH COLLECTION ANALYSIS ---
    ...
    if results_path.exists():
        try:
            ...
            with col2:
                avg_chunks = sum(r["num_chunks"] for r in all_results) / len(
                    all_results
                )
            ...
            with col4:
                papers_with_metadata = sum(
                    1 for r in all_results if r.get("metadata", {}).get("title")
                )
            ...
            with col1:
                ...
                paper_types = list(
                    set(
                        r.get("metadata", {}).get("paper_type", "unknown")
                        for r in all_results
                    )
                )
                selected_type = st.selectbox(
                    "Filter by Paper Type", ["All"] + paper_types
                )
            ...
            with col3:
                ...
                selected_author = st.selectbox(
                    "Filter by Author", ["All"] + unique_authors[:20]
                )  # Limit to first 20
            ...
            if selected_type != "All":
                filtered_results = [
                    ...
                    if r.get("metadata", {}).get("paper_type") == selected_type
                ]
            if selected_year != "All":
                filtered_results = [
                    r
                    for r in filtered_results
                    if r.get("metadata", {}).get("date") == selected_year
                ]
            if selected_author != "All":
                filtered_results = [
                    r
                    for r in filtered_results
                    if selected_author in r.get("metadata", {}).get("authors", [])
                ]
            ...
            for r in filtered_results:
                ...
                paper_data = {
                    ...
                    "Affiliations": (
                        ", ".join(metadata.get("affiliations", [])[:2])
            ...
            for i, paper in enumerate(
                filtered_results[:5]
            ):  # Show first 5 papers in detail
                metadata = paper.get("metadata", {})
                with st.expander(
                    f"📄 {metadata.get('title', paper['paper_name'])}", expanded=False
                ):
                    ...
                    with col2:
                        ...
                        st.write(
                            f"**Keywords:** {', '.join(metadata.get('keywords', [])[:5])}"
                        )
                        st.write(
                            f"**Technical Terms:** {len(metadata.get('technical_terms', []))}"
                        )
                    ...
                    if metadata.get("sections"):
                        st.write("**Sections:**")
                        for section_name, section_text in list(
...
# --- CITATION NETWORK ANALYSIS ---
    ...
    if st.session_state.results_loaded:
        ...
        if network_path.exists():
                ...
                if "cross_references" in network_data:
                    ...
                    if network_data["cross_references"]["direct_citations"]:
                        ...
                        citations_df = pd.DataFrame(
                            network_data["cross_references"]["direct_citations"]
                        )
                    ...
                    if network_data["cross_references"]["shared_topics"]:
                        ...
                        topics_df = pd.DataFrame(
                            network_data["cross_references"]["shared_topics"]
                        )
            ...
            st.warning(
                "Network analysis not found. Process the research collection first."
            )
    else:
    ...
    if st.session_state.results_loaded:
        try:
            ...
            topic_df = pd.DataFrame(
                list(topics.items()), columns=pd.Index(["Topic", "Count"])
            )
            topic_df = topic_df.sort_values("Count", ascending=False)

            fig = px.bar(
                topic_df, x="Topic", y="Count", title="Research Topics Frequency"
            )
            st.plotly_chart(fig)
            ...
            if cooc_matrix:
                cooc_df = pd.DataFrame(
                    [(k[0], k[1], v) for k, v in cooc_matrix.items()],
    ...
    if st.session_state.orchestrator:
        ...
        with st.form("rag_search_form"):
            question = st.text_input(
                ...
                placeholder="e.g., What are the main trends in neuro-symbolic AI?",
            )
        ...
        if submitted:
            if question.strip():
                with st.spinner("Retrieving and synthesizing answer..."):
                    try:
                        result = (
                            st.session_state.orchestrator.rag_system.answer_question(
                        ...
                        if result["relevant_papers"]:
                            for i, paper in enumerate(result["relevant_papers"], 1):
                                paper_title = paper["metadata"].get(
                                    "title",
                                    paper["metadata"].get(
                                        "paper_name", f"Document {i}"
                                    ),
                                )
                                with st.expander(f"Document {i}: {paper_title}"):
                                    st.write(
                                        f"**Paper Name:** {paper['metadata'].get('paper_name', 'N/A')}"
                                    )
                                    if paper["content"] and paper["content"].strip():
                                        ...
                                        if content.count("Key points:") > 1:
                                            ...
                                            first_key_points = content.find(
                                                "Key points:"
                                            )
                                            if first_key_points > 0:
                                                content = (
                                                    content[: first_key_points + 50]
                                                    + "..."
                                                )
                                    ...
                                    if paper.get("distance") is not None:
                                        st.write(
                                            f"**Relevance Score:** {paper['distance']:.3f}"
                                        )
                        else:
                            st.warning("No relevant documents found.")

                        st.success(
                            f"✅ Retrieved {result['num_papers_retrieved']} relevant documents."
                        )
                    except Exception as e:
                        st.error(f"Error during RAG processing: {str(e)}")
                        st.info(
                            "Try rephrasing your question or reducing the number of documents to retrieve."
                        )
            else:
...
if page == "🎯 RL (Reinforcement Learning)":
    ...
    if st.session_state.orchestrator:
        ...
        with col1:
            ...
            if st.button("🎯 Get RL Recommendations"):
                with st.spinner("Training RL agent and generating recommendations..."):
                    try:
                        training_result = st.session_state.orchestrator.train_rl_system(
                            episodes=30
                        )
                        if training_result["training_completed"]:
                            st.success("✅ RL agent trained successfully!")
                            recommendations = st.session_state.orchestrator.rl_recommender.recommend_papers(
                                num_recommendations=num_recommendations
                            )
                            st.subheader("🎯 Recommended Papers")
                            for i, rec in enumerate(recommendations, 1):
                                with st.expander(
                                    f"{i}. {rec['paper_name']} (Confidence: {rec['confidence']:.3f})"
                                ):
                                    st.write(f"**Title:** {rec['title']}")
                                    st.write(
                                        f"**Confidence Score:** {rec['confidence']:.3f}"
                                    )
        ...
        st.markdown("---")
        with st.expander(
            "ℹ️ How does RL Recommendation work in SciSynth?", expanded=False
        ):
...
if page == "🧬 Evolutionary AI":
    ...
    if st.session_state.orchestrator:
        ...
        with col1:
            ...
            if st.button("🧬 Generate Hypotheses (Evolutionary)"):
                with st.spinner("Running evolutionary algorithm..."):
                    try:
                        hypotheses = st.session_state.orchestrator.evo_generator.generate_diverse_hypotheses(
                            num_hypotheses=num_hypotheses
                        )
                        st.subheader("🧬 Generated Hypotheses")
                        for i, hyp in enumerate(hypotheses, 1):
                            with st.expander(
                                f"Hypothesis {i} (Fitness: {hyp['fitness']:.3f})"
                            ):
        ...
        st.markdown("---")
        with st.expander(
            "ℹ️ How does Evolutionary Hypothesis Generation work in SciSynth?",
            expanded=False,
        ):
...
if page == "🧠 Agentic AI & Planning":
    ...
    if st.session_state.orchestrator:
        research_goal = st.text_area(
            ...
            placeholder="e.g., Identify the most promising directions for AI research in the next 5 years",
            height=100,
        )
                ...
                with st.spinner("Planning and executing research task..."):
                    try:
                        ...
                        if cache_key in st.session_state:
                            ...
                            result = st.session_state.orchestrator.cognitive_planner.plan_research_task(
                                research_goal
                            )
                        ...
                        for step_name, step_result in result["results"].items():
                            with st.expander(
                                f"Results: {step_name.replace('_', ' ').title()}",
                                expanded=True,
                            ):
        ...
        st.markdown("---")
        with st.expander(
            "ℹ️ How does Agentic AI Planning work in SciSynth?", expanded=False
        ):
...
def check_consistency(hypothesis: str) -> bool:
    ...
    st.info(
        "Citation network and logical inference are visualized in the Citation Network Analysis section."
    )
    st.markdown("---")
    with st.expander(
        "ℹ️ How does Symbolic & Neuro-Symbolic AI work in SciSynth?", expanded=False
    ):
        st.markdown(
            get_paradigm_explanation("Symbolic AI")
            + "\n---\n"
...
if page == "📋 Capabilities Dashboard":
    ...
    if st.session_state.orchestrator:
        ...
        for ai_type, details in capabilities.items():
            with st.expander(
                f"🤖 {ai_type.upper()} - How it works in SciSynth", expanded=False
            ):
        ...
        st.warning(
            "Orchestrator not initialized. Please process research collection first."
        )
...
st.markdown("---")
st.markdown(
    "*SciSynth: AI Research Assistant - Helping researchers analyze and synthesize scientific literature*"
)
