...
def test_enhanced_data():
    ...
    try:
        ...
        authors_extracted = sum(
            1 for p in papers if p.get("metadata", {}).get("authors")
        )
        keywords_extracted = sum(
            1 for p in papers if p.get("metadata", {}).get("keywords")
        )
        terms_extracted = sum(
            1 for p in papers if p.get("metadata", {}).get("technical_terms")
        )
