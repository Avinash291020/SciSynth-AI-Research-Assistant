    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith('Title:'):
            for j in range(i + 1, min(i + 6, len(lines))):
                ...
                affil_matches = re.findall(r'([A-Z][A-Za-z&\-\. ]*(University|Institute|Department|Inc|Ltd|Lab|School|Center|College|Faculty|Hospital|Company|Corporation|Research|Academy)[A-Za-z&\-\. ]*)', author_line)
...
def extract_keywords(insights):
    ...
    if "Keywords:" in insights:
        ...
        keywords = [kw.strip() for kw in kw_section.split(",") if kw.strip() and len(kw) <= 50]
    if not keywords:
        ai_keywords = [
            'neural networks', 'deep learning', 'machine learning', 'artificial intelligence',
            'AI', 'language models', 'transformers', 'reinforcement learning',
            'evolutionary algorithms', 'symbolic AI', 'neuro-symbolic', 'NLP',
            'computer vision', 'optimization', 'inference', 'training'
        ]
...
def extract_technical_terms(insights, hypotheses):
    ...
    for match in re.finditer(scientific_pattern, text):
        ...
        if len(term) > 3 and any(c.isdigit() for c in term):
            terms.add(term)
    common_words = {'the', 'and', 'for', 'with', 'that', 'this', 'from', 'have', 'been', 'they', 'their'}
...
def extract_sections_and_text(insights):
    ...
    section_headers = [
        'abstract', 'introduction', 'background', 'related work', 'methods', 'methodology', 'approach',
        'results', 'experiments', 'discussion', 'conclusion', 'references', 'acknowledgments', 'future work'
    ]
    lines = insights.split('\n')
    ...
    if "references" in text.lower():
        ...
        for line in refs_text.split('\n')[1:]:
            ...
            if not line or len(line) < 10:
                continue
            if line.lower().startswith(('abstract', 'introduction', 'conclusion', 'keywords')):
                break
...
def main():
    ...
    for i, paper in enumerate(papers):
        print(f"  Processing paper {i+1}/{len(papers)}: {paper.get('paper_name', 'Unknown')}")
